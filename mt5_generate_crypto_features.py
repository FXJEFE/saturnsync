...
Crypto feature generation for BTCUSD, ETHUSD, XRPUSD

Pulls MAXIMUM available history across multiple timeframes (D1, H4, H1, M15, M5)
to build a large training dataset. BTC has ~15 years of D1 data, ETH/XRP ~10 years.

Output: data/FXJEFE_Crypto_Features.csv
"""
import os
import sys
import json
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(config['log_path'], 'mt5_generate_crypto_features.log'), encoding='utf-8'),
...
# Pull max bars per timeframe (MT5 returns up to ~10K per call, we chunk further)
TIMEFRAMES = {
    'D1':  mt5.TIMEFRAME_D1,
    'H4':  mt5.TIMEFRAME_H4,
    'H1':  mt5.TIMEFRAME_H1,
...
MAX_CHUNKS = 10  # up to 100K bars per timeframe
...
def pull_chunked(symbol: str, tf_enum: int) -> pd.DataFrame:
    """Pull data in chunks to get maximum history."""
    all_bars = []
    for chunk in range(MAX_CHUNKS):
        offset = chunk * MAX_BARS_PER_CHUNK
        bars = mt5.copy_rates_from_pos(symbol, tf_enum, offset, MAX_BARS_PER_CHUNK)
        if bars is None or len(bars) == 0:
            break
        all_bars.append(pd.DataFrame(bars))
        if len(bars) < MAX_BARS_PER_CHUNK:
            break
    if not all_bars:
        return pd.DataFrame()
...
def compute_features(df: pd.DataFrame, symbol: str) -> pd.DataFrame:
    """Compute all 27 model features from OHLCV data."""
    ...
    df['vwap']            = ((c * v).cumsum() / vwap_cum_vol.replace(0, np.nan)).fillna(c)
    ...
    output_cols = ['time', 'symbol', 'timeframe', 'price',
...
def main():
    logging.info("mt5_generate_crypto_features.py started")
    ...
    for sym in SYMBOLS:
        ...
        for tf_name, tf_enum in TIMEFRAMES.items():
            logging.info(f"Pulling {sym} {tf_name} (max {MAX_BARS_PER_CHUNK * MAX_CHUNKS} bars)...")
            df = pull_chunked(sym, tf_enum)
            if df.empty:
                logging.warning(f"  No data for {sym} {tf_name}")
                continue
            ...
            df = compute_features(df, sym)
            logging.info(f"  {sym} {tf_name}: {len(df)} rows, "
    ...
    if not all_frames:
        logging.error("No data for any symbol.")
        sys.exit(1)

    result = pd.concat(all_frames, ignore_index=True)
    logging.info(f"Total: {len(result)} rows across {len(all_frames)} symbol-timeframe combos.")

    for sym in SYMBOLS:
        sym_data = result[result['symbol'] == sym]
        for tf in sym_data['timeframe'].unique():
            tf_data = sym_data[sym_data['timeframe'] == tf]
            logging.info(f"  {sym} {tf}: {len(tf_data)} rows")

    output_path = os.path.join(config['data_path'], 'FXJEFE_Crypto_Features.csv')
    ...
    for label, folder_key in [('Common', 'mt5_common_path'), ('Terminal', 'mt5_files_path')]:
        ...
        dest = os.path.join(folder, 'FXJEFE_Crypto_Features.csv')
        try:
            shutil.copy2(output_path, dest)
            logging.info(f"Copied -> {dest}")
        except PermissionError:
            logging.warning(f"{label} copy skipped -- file locked.")

    logging.info("mt5_generate_crypto_features.py completed.")
